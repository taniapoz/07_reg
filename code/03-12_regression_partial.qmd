...
- Complete formal analysis of four models:
  ...
  - linear (intercept + slope)  -> linear model
  - quadratic  -> linear model
  - linear-plateau  -> non linear model
...
Intercept -> yield is not changing, optimum input is 40 
Linear -> as input is higher yield is also higher, the best one is 120 as we have not reached a plateau
Quadratic -> it starts decreasing at some point, after 80 there is lost of yield
Linear-plateau -> it plateaus after 80
...
sr_ksha is numerical, so we can change it to factor, but taht is not what we want to do, meanion geom_boxplot is not a good fit to understand our data
...
mod1_int_boots <- bootMer(mod1_int, 
                          predict.fun, 
                          nsim = 200) %>% #bootstrapping 200 times, at least 1000 when doing research
  confint() %>%
  as.data.frame() %>%
...
```
sr_ksh is numerical so it will give a slope, the slope is 0.05
...
Is best to check the residual for each model becaus ethey change
